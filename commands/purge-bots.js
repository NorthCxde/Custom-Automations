...
module.exports = {
    ...
    async executeInteraction({ client, interaction }) {
        ...
        try {
            ...
            const candidates = messages.filter(msg => msg.author.bot === true).first(count);

            if (candidates.length === 0) {
                return interaction.reply({ content: 'No recent messages from bots were found to delete.', ephemeral: true });
            }
            ...
            await client.addModLog(interaction.guild.id, {
                ...
                reason: `Deleted ${candidates.length} messages from bots`,
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: 'Filter', value: 'Messages from bots', inline: false },
                    { name: 'Target IDs', value: 'N/A', inline: false }
                )
                .setTimestamp();

            await client.logToChannel(interaction.guild, { embeds: [embed] });
            return interaction.reply({ content: `Purged ${candidates.length} message(s) from bots.`, ephemeral: true });
        } catch (error) {
            console.error(error);
